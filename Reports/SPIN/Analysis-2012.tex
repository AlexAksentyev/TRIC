...
We estimate $\slp$ by fitting a linear model to logarithmized beam current data: $\ln I_t = \ln I_0 + \slp\cdot t + \epsilon_t$; therefore, the data must satisfy the four Gauss-Markov assumptions in order for the slope estimator to be BLUE:
\begin{enumerate}
...
Linearity is crucial for the validity of regression, while endogeneity causes bias in the estimate.

Heteroskedasticity and serial correlation do not cause estimator bias; however, they make it inefficient. This problem can be dealt with by generalized least squares. 
...
In the favorable case of a constant offset, it still must be removed in order to have a constant slope to estimate. A time-dependent offset violates the strict exogeneity assumption, and hence biases the slope estimates. At this stage, estimation was done assuming offset was constant within a cycle, and only changed from cycle to cycle.
