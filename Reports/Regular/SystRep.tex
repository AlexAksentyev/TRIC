...
In Figure~\ref{fig:HoldingFld} we drew a schematic representation of the structure of the holding field. We cannot remove the field lines' curvature toward the cell edges, but we don't to either, so long as it is symmetric relative the horizontal plane. 
%Assuming radial symmetry, the vertical component of the induction vector will increase with the radius: $B_y(y) = \alpha\cdot y$. 
...
There are two possible ways to perform the time-reversed $\vec{p}\vec{d}$ scattering: we can flip the beam's polarization, or we can flip the target's. If we do the latter, this is accomplished by inverting the magnetic field's direction. However, in this case we would have to ensure that the inverted field's deviation from the desired direction is not systematically different from the original. If it is (and it's unlikely it is not), the scattering efficiencies of the two cases would be different, and a bias would be introduced.
...
\end{equation}
where $F(f) = \frac{1}{\Thick[T]}\int_{\Lcell}\rho(z) f(z)~ \td z$, $\chi^c$ a measure of the magnetic field strength, $A(\chi^c)$ is the proportionality coefficient between the magnetic field strength and the target polarization.
